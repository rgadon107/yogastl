...
add_action( 'genesis_before_header', __NAMESPACE__ . '\utility_pro_skip_links', 5 );
/**
...
function utility_pro_skip_links() {
	...
	// Added support to include secondary menu in list of skip-links.
	if ( has_nav_menu( 'secondary' ) ) {
		echo '  <li><a href="#genwpacc-genesis-nav-secondary" class="screen-reader-shortcut">'. esc_html__( 'Skip to secondary navigation', 'utility-pro' ) .'</a></li>' . "\n";
	}
...
function utility_skiplinks_markup() {
	...
	add_filter( 'genesis_attr_nav-primary', __NAMESPACE__ . '\utility_pro_genesis_attr_nav_primary' );
	add_filter( 'genesis_attr_nav-secondary', __NAMESPACE__ . '\utility_pro_genesis_attr_nav_secondary' );  // Added filter for secondary nav menu.
	add_filter( 'genesis_attr_content', __NAMESPACE__ . '\utility_pro_genesis_attr_content' );
	add_filter( 'genesis_attr_sidebar-primary', __NAMESPACE__ . '\utility_pro_genesis_attr_sidebar_primary' );
	add_filter( 'genesis_attr_footer-widgets', __NAMESPACE__ . '\genesis_attr_footer_widgets' );
	add_filter( 'genesis_attr_nav-footer', __NAMESPACE__ . '\utility_pro_genesis_attr_nav_footer' );
}
 ...
 * @since 1.0.0
 *
 * @param array $attributes Existing secondary nav attributes.
 * @return array Amended secondary nav attributes.
 */
function utility_pro_genesis_attr_nav_secondary( $attributes ) {
	$attributes['id'] = 'genwpacc-genesis-nav-secondary';
	return $attributes;
}
